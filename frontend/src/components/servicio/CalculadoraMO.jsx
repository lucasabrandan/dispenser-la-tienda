                ...
                <div className="p-3 rounded-xl bg-[#E8E5E0]/50 dark:bg-[#2E2E2E]/50 space-y-3">
                    <div className="p-3 rounded-xl bg-[#FFFFFF]/50 dark:bg-[#242424]/50 space-y-1.5">
                        ...
                        <div className="flex justify-between text-body">
                            ...
                            <span className="font-black text-[#16A34A]">${desglose.efectivoTotal.toLocaleString('es-AR')} <span className="text-caption opacity-60">(-21%)</span></span>
                        </div>
                    </div>
                    <div className="p-3 rounded-xl bg-[#D48800]/10 space-y-1.5">
                        <p className="text-label font-black text-[#D48800] uppercase mb-1">{esVisita ? 'Te queda' : 'Cada técnico'}</p>
                        <div className="flex justify-between text-body">
                    ...
                    <button type="button" onClick={() => setModoReverso(!modoReverso)}
                        className="text-label font-black text-brand-amber uppercase">
                        {modoReverso ? '▲ Cerrar' : '▼ Quiero ganar X → cuánto cobro?'}
                    </button>
                    {modoReverso && (
                        <div className="space-y-2">
                            <div className="flex items-center gap-2">
                                <span className="text-label text-secondary whitespace-nowrap">Quiero {esVisita ? '' : 'c/u '}mínimo:</span>
                                <div className="flex items-center gap-1 flex-1">
                                    <span className="text-body font-black text-ink">$</span>
                                    <input type="text" inputMode="decimal" value={gananciaDeseada}
                                        onChange={e => setGananciaDeseada(e.target.value)} placeholder="30000"
                                        className="flex-1 px-2 py-1.5 rounded-lg text-body font-bold outline-none bg-chip text-ink border border-black/[0.08] dark:border-white/[0.08]" />
                                </div>
                            </div>
                            {reverso && (
                                <div className="p-2.5 rounded-xl bg-[#D48800]/10 space-y-1">
                                    ...
                                    <button type="button"
                                        ...
                                        className="w-full mt-1 py-2 rounded-lg text-label font-black uppercase text-white bg-[#D48800] active:scale-95">
