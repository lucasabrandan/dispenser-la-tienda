    ...
    return (
        <div className="fixed inset-0 z-[2000] flex items-end bg-black/50" onClick={onCerrar}>
            <div className="w-full md:max-w-lg md:mx-auto rounded-t-3xl p-5 max-h-[80vh] flex flex-col bg-card"
                ...
                <p className="text-[11px] text-muted mb-4">📍 {servicio.sedeNombre} · {servicio.fecha}</p>
                    ...
                    {servicio.items?.map((it, idx) => (
                        <div key={idx} className="p-4 rounded-2xl bg-panel">
                            <div className="flex justify-between mb-1">
                                ...
                                <M valor={Number(it.costo || 0)} className="font-black text-[14px] text-ink" />
                            </div>
                            <p className="text-[12px] text-secondary mb-2">{it.trabajoRealizado}</p>
                            {it.repuestosUsados?.length > 0 && (
                                <p className="text-[10px] text-muted pt-2 border-t border-black/[0.05] dark:border-white/[0.05]">
