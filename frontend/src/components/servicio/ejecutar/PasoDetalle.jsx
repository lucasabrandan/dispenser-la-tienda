        ...
        <>
                ...
                {servicio.items?.map((it, i) => (
                    <div key={`${it.equipoSerial || 'item'}-${i}`} className="p-3 rounded-2xl bg-card border border-black/[0.06]">
                        <div className="flex items-start justify-between gap-2 mb-1">
                            <p className="text-[13px] font-black text-brand-red">
                                {it.equipoSerial}
                            </p>
                            {it.equipoModelo && (
                                <p className="text-[10px] text-muted shrink-0">{it.equipoModelo}</p>
                            )}
                        </div>
                        {it.equipoUbicacion && (
                        ...
                        {it.repuestosUsados?.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2 pt-2 border-t border-black/[0.06]">
                                {it.repuestosUsados.map((r, ri) => (
                                    <span key={r.nombre || ri} className="text-[9px] px-1.5 py-0.5 rounded bg-chip text-secondary">
            ...
            <section className="space-y-2">
                ...
                <button type="button" onClick={() => setSheetRepuestosOpen(true)}
                    className="w-full py-3 px-4 rounded-xl flex items-center justify-between font-bold text-[13px] border border-dashed border-chip text-ink bg-[#FFFFFF] dark:bg-[#1C1C1C] active:scale-[0.98] transition-all">
                    <span>
                        ...
                        {repuestosAgregados.map((r, i) => (
                            <div key={r.id ?? i}
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-[10px] text-muted">{fmt(Number(r.precio))} c/u</p>
                                </div>
                                <span className="text-[11px] font-black text-ink shrink-0">x{r.cantidad}</span>
                                <span className="text-[11px] font-black text-brand-red shrink-0 w-16 text-right">
            ...
            <section className="space-y-2">
                <p className="text-[10px] font-black text-muted uppercase tracking-widest">Observaciones</p>
                <textarea
                    value={observaciones}
                    onChange={e => setObservaciones(e.target.value)}
                    placeholder="Anota cualquier detalle del trabajo realizado..."
                    rows={3}
                    className="w-full px-3 py-2.5 rounded-xl text-[13px] border border-black/[0.08] dark:border-white/[0.08] bg-[#FFFFFF] dark:bg-[#2E2E2E] text-ink placeholder:text-muted outline-none resize-none"
                />
            </section>
