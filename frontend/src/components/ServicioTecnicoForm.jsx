...
/**
 * ServicioTecnicoForm - Flujo completo para SERVICIOS TÉCNICOS
 * - Cliente → Sede → Equipo
 * - Repuestos + MO
 * - Garantía automática
 */
export default function ServicioTecnicoForm({ onSaved, servicioParaEditar = null }) {
  const {
    db, setDb,
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-4 pb-48">
      ...
      {idEdicion && !estaBloqueado && (
        <div className="text-center font-black text-rose-500 bg-rose-50 dark:bg-rose-900/20 py-2 rounded-lg mb-4 text-xs uppercase">
          ✏️ Editando Servicio #{idEdicion}
        </div>
      )}
 
      <Card className="mb-4">
        <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-2">🔧 SERVICIO TÉCNICO</h1>
        <p className="text-xs text-slate-400">Con garantía automática y análisis de ganancia</p>
      </Card>
      ...
      <Card className="shadow-sm mb-4">
        ...
        {clienteObj && (
          <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800 grid grid-cols-2 gap-2">
            {clienteObj.telefono && <div><p className="text-[9px] font-black text-slate-400 uppercase">Teléfono</p><p className="text-xs font-bold text-slate-700 dark:text-white">{clienteObj.telefono}</p></div>}
            {clienteObj.localidad && <div><p className="text-[9px] font-black text-slate-400 uppercase">Localidad</p><p className="text-xs font-bold text-slate-700 dark:text-white">{clienteObj.localidad}</p></div>}
          </div>
        )}
      </Card>
      ...
      {clienteId && (
        <Card className="shadow-sm mb-4">
          {/* SEDE */}
          <div className="mb-5">
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Sede / Domicilio</label>
            <CreatableSelect
              isDisabled={estaBloqueado}
              styles={premiumStyles}
          ...
          {/* EQUIPO */}
          <div className="mb-5">
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">S/N Dispenser</label>
            <CreatableSelect
              isDisabled={estaBloqueado}
              styles={premiumStyles}
            ...
            {historialEquipo && (
              <div className="mt-3 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-[10px] font-black text-blue-500 uppercase">Antecedentes</span>
                  <button onClick={enviarWhatsAppMantenimiento} className="bg-emerald-500 text-white px-3 py-1.5 rounded-lg text-[10px] font-black">💬 WHATSAPP</button>
                </div>
          ...
          {!estaBloqueado && (
            <div className="flex gap-2 items-end mb-5">
              <div className="flex-1">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Agregar Repuesto</label>
                <Select
                  styles={premiumStyles}
                  ...
                  formatOptionLabel={opt => (
                    <div className="flex justify-between items-center">
                      <div>{opt.sku && <span className="text-[9px] font-black text-blue-400 mr-2">{opt.sku}</span>}<span className="font-bold">{opt.nombre}</span></div>
              ...
              <button onClick={sumarRepuesto} className="h-[55px] w-14 bg-slate-900 dark:bg-blue-600 text-white rounded-xl text-2xl font-black">+</button>
            </div>
          )}
                ...
                return (
                  <div key={i} className="py-2 border-b border-slate-200 dark:border-slate-700 last:border-0">
                    <div className="flex justify-between items-center">
                      ...
                      <input disabled={estaBloqueado} type="number" value={r.cantidad} min="1" onChange={e => actualizarCantidad(i, e.target.value)} className="w-12 h-9 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg text-center font-black mr-3" />
                    ...
                    {g.ganancia > 0 && (
                      <div className="flex gap-3 mt-1 ml-1 text-[9px]">
                        <span className="font-black text-emerald-500">Ganancia: ${g.ganancia.toFixed(0)}</span>
                        <span className="font-bold text-slate-400">Margen: {g.margen}%</span>
                      </div>
                    )}
                  </div>
                );
          ...
          {/* MANO DE OBRA */}
          <div className="bg-slate-900 dark:bg-slate-800 p-5 rounded-2xl mb-5">
            <label className="text-[10px] font-black text-slate-400 uppercase">Mano de Obra ($)</label>
            <input disabled={estaBloqueado} type="number" min="0" value={itemActual.costoExtra} onChange={e => setItemActual({ ...itemActual, costoExtra: Math.max(0, parseFloat(e.target.value) || 0) })} className="w-full bg-transparent border-none text-white text-4xl font-black outline-none mt-1" />
          </div>
 
          {!estaBloqueado && (
            <button onClick={agregarAlTicket} className="w-full h-14 bg-blue-600 text-white rounded-xl font-black text-sm shadow-lg">
              SUMAR AL TICKET +
            </button>
          )}
        </Card>
      ...
      {ticketItems.length > 0 && !estaBloqueado && (
        <Card className="shadow-sm mb-4">
          ...
          <div className="flex items-center gap-3">
            <input type="number" min="0" max="100" value={descuentoPorcentaje} onChange={e => setDescuentoPorcentaje(Math.min(100, Math.max(0, parseFloat(e.target.value) || 0)))} className="w-24 h-12 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-center font-black text-2xl" />
          ...
          <div className="flex gap-2 mt-3">
            {[5, 10, 15, 20].map(p => (
              <button key={p} onClick={() => setDescuentoPorcentaje(p)} className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase ${descuentoPorcentaje === p ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-800'}`}>
                {p}%
              </button>
            ))}
          </div>
      ...
      {ticketItems.length > 0 && (
        <div className="mt-6 mb-4">
          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Resumen del Remito</h4>
          {ticketItems.map((it, idx) => (
            <div key={idx} className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 mb-3 flex justify-between items-start">
              <div className="flex-1 pr-4">
                <div className="font-black text-sm text-rose-500">{it.equipoSerial}</div>
                <div className="text-xs mt-1.5 text-slate-600 dark:text-slate-400 font-bold">{it.resumenTexto}</div>
                ...
                <div className="text-2xl font-black mt-3 text-slate-900 dark:text-white">${it.totalCalculado.toLocaleString()}</div>
              </div>
              {!estaBloqueado && (
          ...
          {resumen && resumen.gananciaBruta > 0 && (
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-4 mt-2">
              <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-3">📊 Rentabilidad</p>
              <div className="grid grid-cols-2 gap-3">
                <div><p className="text-[9px] font-bold text-slate-400 uppercase">Total Venta</p><p className="font-black text-slate-800 dark:text-white">${resumen.totalVenta.toLocaleString()}</p></div>
                <div><p className="text-[9px] font-bold text-slate-400 uppercase">Costo Total</p><p className="font-black text-slate-800 dark:text-white">${resumen.totalCosto.toFixed(0)}</p></div>
                {resumen.descuento > 0 && <div><p className="text-[9px] font-bold text-slate-400 uppercase">Descuento</p><p className="font-black text-rose-500">- ${resumen.descuento.toFixed(0)}</p></div>}
                <div><p className="text-[9px] font-bold text-slate-400 uppercase">Ganancia</p><p className="font-black text-emerald-600 text-lg">${resumen.gananciaBruta.toFixed(0)}</p></div>
                <div className="col-span-2"><p className="text-[9px] font-bold text-slate-400 uppercase">Margen Final</p><div className="flex items-center gap-2"><div className="flex-1 h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden"><div className="h-full bg-emerald-500" style={{ width: `${Math.min(100, resumen.margenFinal)}%` }} /></div><span className="font-black text-emerald-600">{resumen.margenFinal}%</span></div></div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed bottom-[100px] left-4 right-4 z-[1000]">
          <div className="bg-slate-900 dark:bg-slate-800 p-4 pl-6 pr-4 rounded-3xl flex justify-between items-center shadow-2xl">
            ...
            <div className="flex gap-2">
              ...
              {!estaBloqueado && (
                <>
                  <button onClick={() => handleFinalizar(false)} className="bg-slate-700 text-white px-4 rounded-2xl font-black text-[11px]">{idEdicion ? 'ACTUALIZAR' : 'GUARDAR'}</button>
                  <button onClick={() => handleFinalizar(true)} className="bg-rose-500 text-white px-6 rounded-2xl font-black text-xs">CONFIRMAR</button>
                </>
              )}
            </div>
          </div>
      ...
      {/* MODALES */}
      <CrearClienteModal isOpen={modalClienteAbierto} onClose={() => setModalClienteAbierto(false)} onClienteCreado={async clienteNuevo => { setDb({ ...db, clientes: [...db.clientes, clienteNuevo] }); onClienteSeleccionado(clienteNuevo.id.toString()); setModalClienteAbierto(false); await refrescarDatos(); }} clienteNombrePrellenado={nombreClientePrellenado} />
      <CrearSedeModal isOpen={modalSedeAbierto} onClose={() => setModalSedeAbierto(false)} onSedeCreada={async sedeNueva => { setDb({ ...db, sedes: [...db.sedes, sedeNueva] }); setItemActual({ ...itemActual, sedeId: sedeNueva.id, sedeNombre: sedeNueva.nombreSede }); setModalSedeAbierto(false); await refrescarDatos(); }} clienteId={clienteId} nombreSedePrellenado={nombreSedePrellenado} />
      <CrearEquipoModal isOpen={modalEquipoAbierto} onClose={() => setModalEquipoAbierto(false)} onEquipoCreado={async equipoNuevo => { setDb({ ...db, equipos: [...db.equipos, equipoNuevo] }); setItemActual({ ...itemActual, equipoSerial: equipoNuevo.numeroSerie }); consultarAntecedentes(equipoNuevo.numeroSerie); setModalEquipoAbierto(false); await refrescarDatos(); }} sedeId={itemActual.sedeId} numeroSeriePrellenado={numeroSeriePrellenado} />
    </div>
  );
}