        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-end z-[2000]">
            <div className="bg-card w-full max-w-xl rounded-t-[2rem] shadow-2xl animate-slide-up h-[92vh] flex flex-col">
                ...
                <div className="px-6 pb-4 border-b border-black/[0.07] dark:border-white/[0.07]">
                    ...
                    <p className="text-body font-bold text-muted uppercase mt-1">{cliente.nombre}</p>
                </div>
                ...
                {!equipoParaEditar && (
                    <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
                        {equipos.length === 0 ? (
                            <div className="text-center py-10 font-black uppercase text-caption text-muted border-2 border-dashed border-black/[0.07] dark:border-white/[0.07] rounded-2xl">
                            ...
                            equipos.map(eq => (
                                <div key={eq.id} className="p-4 bg-panel rounded-2xl border border-black/[0.07] dark:border-white/[0.07] flex justify-between items-center">
                                    <div>
                                        ...
                                        <p className="text-caption font-bold text-brand-amber uppercase">
                                            {eq.marca} · {eq.modelo}
                                        </p>
                                    </div>
                                    <span className="text-label font-black bg-card px-3 py-1.5 rounded-xl border border-black/[0.07] dark:border-white/[0.07] uppercase text-muted">
                ...
                >
                    ...
                    <div className="space-y-1">
                        <label className="text-label font-black text-muted uppercase ml-1">Ubicación (Sede)</label>
                    ...
                    {form.marca === 'OTRA' && (
                        <div className="space-y-1">
                            <label className="text-label font-black text-muted uppercase ml-1">Nombre de Marca</label>
                    ...
                    {/* Modelo */}
                    <div className="space-y-1">
                        <label className="text-label font-black text-muted uppercase ml-1">Modelo / Tipo</label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                            <label className="text-label font-black text-muted uppercase ml-1">Ubicación Interna</label>
                            <input className={fieldCls} placeholder="Ej: Recepción"
                                value={form.ubicacion} onChange={e => setForm({ ...form, ubicacion: e.target.value })} />
                        </div>
                        <div className="space-y-1">
                            <label className="text-label font-black text-muted uppercase ml-1">Piso</label>
                            <input className={fieldCls} placeholder="Ej: 3, PB, Sub"
                                value={form.piso} onChange={e => setForm({ ...form, piso: e.target.value })} />
                        </div>
                    </div>
                    <div className="space-y-1">
                        <label className="text-label font-black text-muted uppercase ml-1">Sector / Área</label>
                    ...
                    {/* Notas */}
                    <div className="space-y-1">
                        <label className="text-label font-black text-muted uppercase ml-1">Notas Técnicas</label>
                        <textarea
                    ...
                    <div className="flex gap-3 pt-2">
                        <button type="button" onClick={onClose} disabled={cargando}
                            className="flex-1 h-14 bg-chip text-ink rounded-2xl font-black text-label uppercase hover:opacity-80 transition-all active:scale-95 disabled:opacity-50">
                            Cancelar
                        </button>
                        <button type="submit" disabled={cargando}
                            className="flex-[2] h-14 bg-brand-red text-white rounded-2xl font-black text-label uppercase shadow-lg hover:opacity-90 transition-all active:scale-95 disabled:opacity-50">
                            {cargando
                                ? 'Guardando...'
